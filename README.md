...
- Carga directa (`require`) de todos los módulos backend nuevos/tocados (`g3CatalogClient.js`, `liveIntegrationsSync.js`, `g7InventoryClient.js`, `reports.js`, `batch.js`, `inventory.js`) sin errores de sintaxis/referencia.
- Merge G7↔G3 probado end-to-end contra ambos servicios reales (no mock): 22 productos en inventario de G7, **19 enriquecidos correctamente** con nombre/categoría real de G3. Los 3 restantes son productos de prueba en G7 con UUIDs de ejemplo (`550e8400-...`, `660e8400-...`) nunca registrados en el catálogo real de G3 — el fallback (usar `productId` como nombre, `category: null`) funciona como corresponde, no es un bug.
